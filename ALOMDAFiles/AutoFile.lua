...
DevAbs:del(ALOMDA.."Abs:Lock:AutoFile")
end
...
DevAbs:set(ALOMDA.."Abs:Lock:AutoFile",true) 
end 
end

if (text and not DevAbs:get(ALOMDA.."Abs:Lock:AutoFile")) then
Time = DevAbs:get(ALOMDA.."Abs:AutoFile:Time")
if Time then 
if Time ~= os.date("%x") then 
local list = DevAbs:smembers(ALOMDA..'Abs:Groups') 
local BotName = (DevAbs:get(ALOMDA.."Abs:NameBot") or 'بروكس')
...
BasicConstructors = DevAbs:smembers(ALOMDA..'Abs:BasicConstructor:'..v)
Constructors = DevAbs:smembers(ALOMDA..'Abs:Constructor:'..v)
...
local curl = 'curl "' .. abbas .. '" -F "chat_id='..DevId..'" -F "document=@'..ALOMDA..'.json' .. '" -F "caption=⌁︙نسخه تلقائيه تحتوي على ↫ '..#list..' مجموعه"'
io.popen(curl)
io.popen('fm -fr '..ALOMDA..'.json')
DevAbs:set(ALOMDA.."Abs:AutoFile:Time",os.date("%x"))
end
else 
DevAbs:set(ALOMDA.."Abs:AutoFile:Time",os.date("%x"))
end
